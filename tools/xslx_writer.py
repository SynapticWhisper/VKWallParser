...
def save_to_excel(
    ...
    # При работе с большим количеством данных, сборка всех данных по
    # категориям датафрейм может вызвать проблемы с памятью, с небольшим
    # количеством данных целесообразнее сначала собрать все в DF а потом
    # уже один раз записывать это все в файл (операции с файлами = долго)
    ...
    with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
        for posts_chunk in VkRequests.get_posts(date_limit, domain, owner_id):
            ...
            for post in posts_chunk:
                for comments_chunk in VkRequests.get_comments(
                    post["from_id"],
                    post["id"]
                ):
                    filtered_comments = filter_comment_data(
                        comments_chunk,
                        post["id"]
                    )
                    ...
                    comments_df.to_excel(
                        ...
                        index=False,
                        startrow=comment_row,
                        header=not comment_row
                    )
                    comment_row += len(comments_df)

                for likes_chunk in VkRequests.get_likes(
                    post["from_id"],
                    post["id"]
                ):
